...
method_str = ["GflowDag", "DiBs", "MCMC Gibbs", "MCMC MH", "BS GES", "BS PC"]
result = "results2"
file_path = return_file_paths(0, result, methods[0])["summary_all_data"]
...
for metric in metrics:
    ...
    for method, stri in zip(methods, method_str):
        file_path = return_file_paths(0, result, method)["summary_all_data"]
...
for metric in metrics:
    ...
    print(metric)
    if metric == "negll":
        plot_is = sns.boxplot(data=(dfs[metric].T), showfliers=False)
    ...
    fig.savefig("figs/" + result + "_" + metric + ".png")
